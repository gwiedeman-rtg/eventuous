...
/// <summary>
/// Tests for AVRO parsing in Azure Event Hubs Event Store.
/// These tests ensure that the AVRO parsing fixes are working and would fail if removed.
/// </summary>
public class AvroParsingTests : IClassFixture<IntegrationFixture> {
    readonly IntegrationFixture _fixture;
    readonly ITestOutputHelper _output;

    public AvroParsingTests(IntegrationFixture fixture, ITestOutputHelper output) {
        _fixture = fixture;
        _output = output;
    }

    [Fact]
    public async Task Should_Parse_AVRO_Capture_Files_Correctly() {
        // Arrange - Create a stream with events
        var stream = new StreamName("avro-test-stream");
        ...
        // Act - Read events (this will use AVRO parsing if capture is enabled)
